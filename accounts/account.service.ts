...
async function update(id: any, params: any) {
    ...
    if (account. email && account.email !== params.email && await db.Account. findOne({ where: { email: params.email } })) {
        throw 'Email "' + params.email + '" is already registered';
    }
...
function basicDetails(account: any) {
    const { id, email, title, firstName, lastName, role, isVerified, created, updated } = account;
    return { id, email, title, firstName, lastName, role, isVerified, created, updated };
}
...
async function sendAlreadyRegisteredEmail(email: any, origin: any) {
    ...
    if (origin) {
        ...
        message = `<p>If you don't know your password you can reset it using the <code>/account/forgot-password</code> API route.</p>`;
    }
...
async function sendPasswordResetEmail(account: any, origin: any) {
    ...
    if (origin) {
        ...
        message = `<p>Please use the below token to reset your password with the <code>/account/reset-password</code> API route, the token will be valid for 1 day:</p>
